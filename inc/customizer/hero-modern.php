<?php
/**
 * Hero Section - MODERN design controls
 *
 * Eyebrow / two-line title / feature bullets / product carousel /
 * "Other goods" cards. Registered into the shared "mr_hero" Customizer
 * section by mr_hero_customizer() in inc/customizer/hero.php.
 * Controls only show when Hero Design = Modern (see mr_hero_is_modern()).
 */

if (!function_exists('mr_hero_modern_controls')) {
    function mr_hero_modern_controls($wp_customize) {
    ...
    $wp_customize->add_control(new WP_Customize_Media_Control($wp_customize, 'hero_bg_image', array(
        ...
        'section' => 'mr_hero',
        'mime_type' => 'image',
        'active_callback' => 'mr_hero_is_modern',
        'priority' => 10,
    )));
    ...
    // === FEATURE BULLETS (2) - icon is a media upload ===
    // Feature 1
    ...
    for ($i = 1; $i <= 4; $i++) {
        ...
        $wp_customize->add_control(new WP_Customize_Media_Control($wp_customize, "hero_carousel_image_{$i}", array(
            ...
            'active_callback' => function() {
                return mr_hero_is_modern() && get_theme_mod('hero_show_product_carousel', true);
            },
            'priority' => 90 + ($i * 2) - 1,
        )));
    ...
    foreach ($other_goods_defaults as $i => $defaults) {
        ...
        $wp_customize->add_control("hero_goods_{$i}_title", array(
            ...
            'type' => 'text',
            'active_callback' => function() {
                return mr_hero_is_modern() && get_theme_mod('hero_show_other_goods', true);
            },
