...
function mr_enqueue_auth_scripts() {
    ...
    if (!is_user_logged_in()) {
        ...
        wp_localize_script('mr-auth-modal', 'mr_auth', array(
            ...
            'login_subtitle' => get_theme_mod('auth_modal_login_subtitle', __('Welcome back! Please enter your details', 'macedon-ranges')),
            'register_subtitle' => get_theme_mod('auth_modal_register_subtitle', __('Create your account to get started', 'macedon-ranges')),
            'show_social' => get_theme_mod('auth_modal_show_social', true),
        ));
    }
}
...
function mr_ajax_login() {
    ...
    if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'mr_auth_nonce')) {
        wp_send_json_error(array(
            'message' => __('Security check failed. Please refresh the page and try again.', 'macedon-ranges')
        ));
    }
    ...
    if (empty($username) || empty($password)) {
        wp_send_json_error(array(
            'message' => __('Please fill in all required fields.', 'macedon-ranges')
        ));
    }
    ...
    if (is_wp_error($user)) {
        wp_send_json_error(array(
            'message' => __('Invalid username or password. Please try again.', 'macedon-ranges')
        ));
    }

    // Success
    wp_send_json_success(array(
        'message' => __('Login successful! Redirecting...', 'macedon-ranges'),
...
function mr_ajax_register() {
    ...
    if (!get_option('users_can_register')) {
        wp_send_json_error(array(
            'message' => __('User registration is currently disabled.', 'macedon-ranges')
        ));
    }
    ...
    if (!validate_username($username)) {
        wp_send_json_error(array(
            'message' => __('Invalid username. Only lowercase letters, numbers, and underscores are allowed.', 'macedon-ranges')
        ));
    }
    ...
    if (username_exists($username)) {
        wp_send_json_error(array(
            'message' => __('This username is already taken. Please choose another one.', 'macedon-ranges')
        ));
    }
    ...
    if (!is_email($email)) {
        wp_send_json_error(array(
            'message' => __('Please enter a valid email address.', 'macedon-ranges')
        ));
    }
    ...
    if (email_exists($email)) {
        wp_send_json_error(array(
            'message' => __('This email is already registered. Please use another email or login.', 'macedon-ranges')
        ));
    }
    ...
    if (strlen($password) < 6) {
        wp_send_json_error(array(
            'message' => __('Password must be at least 6 characters long.', 'macedon-ranges')
        ));
    }
    ...
    // Success
    wp_send_json_success(array(
        'message' => __('Registration successful! Welcome aboard!', 'macedon-ranges'),
        'redirect' => get_permalink(get_option('woocommerce_myaccount_page_id'))
    ));
}
