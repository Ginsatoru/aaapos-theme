 ...
 * @version 3.3.0
 *
 * UPDATED (aaapos-prime): This template is shared between two contexts -
 * (1) the single order view page (My Account > Orders > [order]), where
 * it duplicated the dedicated Downloads tab, and (2) the actual Downloads
 * tab listing itself. We only want to suppress it in context (1), so this
 * now exits early ONLY when on the single order view endpoint, and
 * renders normally everywhere else (including the real Downloads tab).
 */
...
// Suppress this table on the single order view page only - it duplicated
// the dedicated My Account > Downloads tab. The Downloads tab itself is
// NOT the "view-order" endpoint, so it still renders normally below.
